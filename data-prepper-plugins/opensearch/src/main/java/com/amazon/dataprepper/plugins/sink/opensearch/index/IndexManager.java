...
public abstract class IndexManager {
    ...
    public final void checkAndCreateIndexTemplate(final boolean isISMEnabled, final String ismPolicyId) throws IOException {
        ...
        final String indexTemplateName = indexPrefix.replaceAll("-$", "")  + "-index-template";
        ...
        putIndexTemplateRequest.patterns(ismPolicyManagementStrategy.getIndexPatterns(indexPrefix));

        if (isISMEnabled) {
            attachPolicy(openSearchSinkConfiguration.getIndexConfiguration(), ismPolicyId, indexPrefix);
        }
